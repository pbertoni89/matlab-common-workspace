%% disegno la direzione di discesa e la sezione di phi lungo la direzione di discesa del gradiente coniugato
% 
%clear;
...
figure(1)
% disegno la direzione d^k
plot3(xx(1,:),xx(2,:),zeros(100,1),'k','linewidth',2) 
% disegno la restrizione di f alla direzione d^k
...
figure(1)
text(1.5,-0.5,0,'${\bf r}^{(k)}=-\nabla \phi({\bf x}^{(k)})$',...
    'Interpreter','Latex')
text(-1.5,-1.7,0,'${\bf x}^{(k)}$',...
    'Interpreter','Latex') 
text(1,-0.7,0,'${\bf x}^{(k+1)}$',...
    'Interpreter','Latex')
text(1.,1.8,0,'${\bf d}^{(k)}$',...
    'Interpreter','Latex') 
