...
for i=1:realiz
    ...
    %% FILTRAGGIO CON QUADRATORE
    Processo(i,:) = Processo(i,:).^2;
end

for j=1:length(istanti)
    X(j,:) = Processo( :, t==istanti(j) );
    str = sprintf('PDF del processo PAM QUADRATO in t=%fs',istanti(j));
    % dalla somiglianza pseudocasuale delle PDF, deduco la STAZIONARIETA' IN SENSO STRETTO
    myHist1D( X(j,:) ); title(str);
    fprintf('La media del PAM QUADRATO all istante %f è %f \n', istanti(j), mean( X(j,:) ) );
...
for i=1:length(istanti)
    ...
    str = sprintf('Autocorrelazione del processo PAM QUADRATO (funzione di t'') centrato in t=%fs',istanti(i));
...
% l'ergodicità rispetto alla media viene persa!
...
phi_x= phi_x / 13;  % normalizzo la cross-correlazione rispetto al periodo (supporto totale del frammento di processo PAM)
...
plot(t, phi_x, 'r' ); title('autocorrelazione del processo');  axis([-10 10 -.5 4])
hold off;
